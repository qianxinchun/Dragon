...
void AdaDeltaSolver<Dtype>::computeUpdateValue(int param_id, Dtype rate){
	...
	case Dragon::CPU:
		...
		//	2. compute history
		dragon_cpu_axpby<Dtype>(count, Dtype(1) - momentum, temp[param_id]->cpu_data(),
		...
		dragon_powx<Dtype>(count, net_param->cpu_diff(), Dtype(2), temp[param_id]->mutable_cpu_data());
		//	8. compute update
		dragon_cpu_axpby<Dtype>(count, (1 - momentum), temp[param_id]->cpu_data(),
			momentum, update[param_id]->mutable_cpu_data());
	...
	case Dragon::GPU:
		...
		adadeltaUpdate(net_param->count(),net_param->mutable_gpu_diff(),
			history[param_id]->mutable_gpu_data(), update[param_id]->mutable_gpu_data(),momentum, eps, lr);
#endif
		break;
