...
Solver<Dtype>::Solver(const SolverParameter& param, const Solver* root_solver = NULL)
	:param(param), root_solver(root_solver), need_early_stopping(false){
	init(param);
}

template <typename Dtype>
Solver<Dtype>::Solver(const string& param_file, const  Solver* root_solver = NULL)
	:root_solver(root_solver), need_early_stopping(false){
...
void Solver<Dtype>::solve(const char* resume_file){
	...
	if (need_early_stopping){
		LOG(INFO) << "Optimization stop early.";
		return;
	}
...
void Solver<Dtype>::test(int net_id){
	...
	for (int i = 0; i < param.test_iter(net_id);i++){
		SolverAction request = getRequestedAction();
		while (request != SolverAction::NONE){
			if (request == SolverAction::SNAPSHOT) snapshot();
			else if (request == SolverAction::STOP) need_early_stopping = true;
	...
	if (need_early_stopping) { LOG(INFO) << "Test is interrupted."; return; }
...
void Solver<Dtype>::testAll(){
	for (int net_id = 0; net_id < test_nets.size(); net_id++){
		if (need_early_stopping) break;
		test(net_id);
	}
}
...
void Solver<Dtype>::step(int iters){
	...
	while (iter < stop_iter){
		...
		if (param.test_interval() && iter%param.test_interval() == 0 &&Dragon::get_root_solver()){
			...
			if ((iter == 0 && param.test_before_train()) || iter != 0){
				testAll();
				if (need_early_stopping) break;
			}
		}
		...
		if (display){
			LOG_IF(INFO, Dragon::get_root_solver())
				<< "Iteration " << iter << ", loss = " << smoothed_loss;
			int score_idx = 0;
			...
			for (int i = 0; i < result.size(); i++){
				...
				for (int j = 0; j < result[i]->count(); j++){
					...
					LOG(INFO) << "		Train net output #" << i << "(" << output_name << "): "
						<< res_vec[j] << msg.str();
				}
			}
		}
		for (int i = 0; i < callbacks.size(); i++) callbacks[i]->on_gradients_ready();
		applyUpdate();
		iter++;
		SolverAction request = getRequestedAction();
		// snapshot if at the time or necessary
		if ((param.snapshot_interval() && iter%param.snapshot_interval() == 0 &&
			Dragon::get_root_solver()) || request == SolverAction::SNAPSHOT)
			snapshot();
		if (request == SolverAction::STOP){
			need_early_stopping = true;
			break;
		}
	}
}
...
template <typename Dtype>
void Solver<Dtype>::setActionFunction(ActionCallBack function){
	action_request_function = function;
}

template <typename Dtype>
SolverAction Solver<Dtype>::getRequestedAction(){
	if (action_request_function) return action_request_function();
	else return SolverAction::NONE;
}
